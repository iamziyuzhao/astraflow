...
R3 (Rollout Routing Replay) attaches an int32 tensor of shape
``[B, S, num_hidden_layers * top_k]`` to every training batch. The helpers in
...
1. ``split_padded_tensor_dict_into_mb_list`` splits ``[B, S, C]`` tensors
   by row alongside ``input_ids`` instead of duplicating them into every
   micro-batch.
2. ``pack_tensor_dict`` -> ``pad_packed_tensor_dict`` ->
   ``unsqueeze_packed_tensor_dict`` preserves per-token ``[C]`` rows aligned
   to packed positions (the unsqueeze is the FSDP path; Megatron installs
   the ``[T_pad, C]`` tensor directly).
3. qwen2.5-vl-style ``position_ids`` still takes its special path
   (regression guard for the predicate change).
4. The padded ``[T_pad, C]`` tensor is what ``RoutingReplayContext``
   installs: pad rows route to expert 0.
"""
...
from astraflow.train_worker.utils.mcore.routing_replay import RoutingReplayContext

NUM_LAYERS = 4
TOP_K = 2
NUM_COLUMNS = NUM_LAYERS * TOP_K
# Distinct value per (layer, expert-slot) column so misaligned copies are caught.
CELL_OFFSETS = torch.arange(NUM_COLUMNS, dtype=torch.int32)
...
def _expected_experts(token_ids: torch.Tensor) -> torch.Tensor:
    ...
    return token_ids.to(torch.int32).unsqueeze(-1) * 10 + CELL_OFFSETS
...
def _assert_rows_aligned(input_ids: torch.Tensor, routed_experts: torch.Tensor):
    """Every position with a nonzero token carries its derived expert row;
    pad positions carry an all-zero row."""
    valid = input_ids != 0
...
class TestSplitPaddedTensorDict:
    def test_routed_experts_split_by_row(self):
        ...
        for mb in mb_list.mbs:
            ...
            assert experts.shape == (n_rows, max(lens), NUM_COLUMNS)
...
class TestPackedRoundTrip:
    def test_pack_pad_unsqueeze_preserves_rows(self):
        ...
        assert experts.shape == (total_length, NUM_COLUMNS)
        ...
        assert experts.shape == (pad_to_length, NUM_COLUMNS)
        assert experts.dtype == torch.int32
        ...
        assert unsqueezed["routed_experts"].shape == (1, pad_to_length, NUM_COLUMNS)
    ...
    def test_pad_packed_align_sequences_preserves_rows(self):
        ...
        experts = padded["routed_experts"]
        assert experts.shape[1:] == (NUM_COLUMNS,)
    ...
    def test_padded_tensor_installs_into_the_replay_context(self):
        """The Megatron path: ``[T_pad, C]`` goes straight to install_packed."""
        lens = [6, 4, 2]
        data = _make_padded_batch(lens)
        data.pop("rewards")
        padded, _, _, _ = pad_packed_tensor_dict(
            pack_tensor_dict(data), pad_to_length=16, align_sequences=True, align_to_multiple_of=4
        )
        experts = padded["routed_experts"]
        tokens = experts.shape[0]

        layer_map = {ln: ln - 1 for ln in range(1, NUM_LAYERS + 1)}
        ctx = RoutingReplayContext([layer_map], NUM_LAYERS, TOP_K)
        ctx.begin_pass(forward_only=True)
        ctx.install_packed(experts, 0)
        view = experts.view(tokens, NUM_LAYERS, TOP_K)
        for layer_number, hf_index in layer_map.items():
            served = ctx.fetch(layer_number, tokens)
            assert torch.equal(served, view[:, hf_index, :].long())
            # pad rows are zero rows: they route to expert 0, as documented
            assert (served[padded["input_ids"] == 0] == 0).all()
...
class TestConcatPaddedTensors:
    def test_mixed_length_nd_concat(self):
        ...
        assert out["routed_experts"].shape == (2, long, NUM_COLUMNS)
        assert out["routed_experts"].dtype == torch.int32
