...
Routed-expert tensors are int32 ``[1, seq_len, num_hidden_layers * top_k]``
-- the padding path must preserve the trailing dim instead of assuming 2D.
"""
...
def _routed_experts_for(input_ids: torch.Tensor) -> torch.Tensor:
    ...
    base = input_ids[0].to(torch.int32).reshape(seq_len, 1)
    offsets = torch.arange(NUM_COLUMNS, dtype=torch.int32)
...
def test_concat_pads_nd_trailing_dims():
    ...
    assert batch["routed_experts"].shape == (3, 5, NUM_COLUMNS)
...
def test_concat_slice_reconcat_round_trip():
    ...
    for i, sliced in enumerate(slices):
        assert sliced["routed_experts"].shape == (1, 5, NUM_COLUMNS)
        assert sliced["routed_experts"].dtype == torch.int32
...
def test_reconcat_with_mixed_max_lengths():
    ...
    assert merged["routed_experts"].shape == (3, 5, NUM_COLUMNS)
