...
dtype, trailing dim, and row order of int32
``[1, seq_len, num_hidden_layers * top_k]`` tensors.
"""
...
def test_put_get_preserves_routed_experts_dtype_shape_order():
    ...
    assert experts.shape == (2, 4, NUM_COLUMNS)
...
def test_put_slices_multi_example_batch_nd():
    ...
    # each entry must keep its full [1, S, C] routed_experts block.
    buffer = RolloutBuffer(max_size=8, queue_order="fifo")
    ...
    assert result is not None
    batch, _ = result
    assert batch["routed_experts"].shape == (2, 3, NUM_COLUMNS)
...
def test_state_dict_round_trip_preserves_routed_experts():
    ...
    batch, _ = result
    assert batch["routed_experts"].dtype == torch.int32
    assert batch["routed_experts"].shape == (1, 3, NUM_COLUMNS)
