#!/bin/bash
set -euo pipefail
# [2/3] Launch RaaS inference server (SGLang + TCP receiver, R3 capture)
#
# Runs on any 4-GPU rollout node (>=80 GB per GPU). For the remote 4xB200
# node use 2b_raas_b200.sh instead. If this node is not the trainer node,
# export ASTRAFLOW_URL=http://<trainer-host>:8000 first.
#
