#!/bin/bash
# AMD/ROCm launcher for the llama3.2-3b-instruct-m2po-delta recipe (MI300/MI325, gfx942).
#
...
#   bash examples/math/llama3.2-3b-instruct-m2po-delta/scripts/run_llama3.2-3b-instruct-m2po-delta_amd.sh
# Under Slurm (pyxis-free; uses the node's docker daemon):
#   srun --gres=gpu:8 --nodes=1 --ntasks=1 bash examples/math/.../run_llama3.2-3b-instruct-m2po-delta_amd.sh
#
# Env knobs:
...
bash examples/math/llama3.2-3b-instruct-m2po-delta/scripts/run_llama3.2-3b-instruct-m2po-delta.sh ${OVERRIDES[*]}
INNER_EOF
